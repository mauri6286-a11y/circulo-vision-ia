...
const r13 = StateMachine.processMessage('c13', 'tienen convenio con caja bancaria?');
...
const r14 = StateMachine.processMessage('c14', 'tienen descuento con fitlab o sayago?');
...
const r15 = StateMachine.processMessage('c15', 'cuanto demoran en entregar los lentes?');
...
const r16 = StateMachine.processMessage('c16', 'los lentes tienen garantia?');
assert(r16.reply.includes("60 días"), '16. Adaptation Guarantee Query');

// 17. ENVÍOS A DOMICILIO Y TOMA DE MEDIDAS EN LOCAL
const r17 = StateMachine.processMessage('c17', 'hacen envios al interior o a domicilio?');
...
const r18 = StateMachine.processMessage('c18', 'tienen cuotas sin recargo con tarjeta?');
assert(r18.reply.includes("12 cuotas sin recargo"), '18. Credit Card Installments Query');

// 19. TEST VISUAL 100% GRATIS
const r19 = StateMachine.processMessage('c19', 'cuanto sale el examen de vista?');
assert(r19.reply.includes("100% gratis"), '19. Test Visual Zero Cost Query');

// 20. CONVENIO CÍRCULO CATÓLICO (15% efectivo)
const r20 = StateMachine.processMessage('c20', 'descuento circulo catolico');
...
const r21 = StateMachine.processMessage('c21', 'descuento hospital evangelico');
...
const r22 = StateMachine.processMessage('c22', 'descuento stiq sindicato');
assert(r22.reply.includes("Sindicato de la Industria Química") && r22.reply.includes("20% efectivo"), '22. Active Convenio Query (STIQ)');

// 23. CONVENIO BPS (Subsidio BPS)
const r23 = StateMachine.processMessage('c23', 'aceptan bps recetas con subsidio?');
assert(r23.reply.includes("Subsidio BPS"), '23. Active Convenio Query (BPS)');

// 24. PRECIO BLUEBLOCKER / LUZ AZUL ($3.200)
const r24 = StateMachine.processMessage('c24', 'cuanto sale el blueblocker para computadoras?');
assert(r24.reply.includes("$3.200"), '24. Blueblocker Price Query ($3.200)');

// 25. PRECIO CRISTAL BLANCO ($1.300)
const r25 = StateMachine.processMessage('c25', 'cuanto sale el cristal blanco comun?');
assert(r25.reply.includes("$1.300"), '25. Blanco Cristal Price Query ($1.300)');

// 26. LENTES DE SOL (UV400 / Polarizados)
const r26 = StateMachine.processMessage('c26', 'tienen lentes de sol polarizados?');
assert(r26.reply.includes("UV400") && r26.reply.includes("polarizados"), '26. Sunglasses Query (UV400)');

// 27. AGRADECIMIENTOS
const r27 = StateMachine.processMessage('c27', 'muchas gracias impecable!');
assert(r27.reply.includes("Quedamos a las órdenes"), '27. Gratitude Response');

// 28. DESPEDIDAS SECUNDARIAS
const r28 = StateMachine.processMessage('c28', 'saludos que pases bien');
assert(r28.reply.includes("Muchas gracias a ti"), '28. Goodbye Response');

// 29. NO TENGO RECETA / REPITE YA TE DIJE
const r29 = StateMachine.processMessage('c29', 'ya te dije que no tengo receta');
...
const r30 = StateMachine.processMessage('c30', 'que garantia especifica tiene este modelo?');
